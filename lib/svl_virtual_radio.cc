...
VirtualRadio::add_iq_sample(const gr_complex *samples, size_t len)
{
	...
	LOG(INFO) << "VR:" << g_idx << " - g_tx_samples.size():" << g_tx_samples.size();
}
...
VirtualRadio::map_iq_samples(gr_complex *samples_buf)
{
   ...
   std::copy(g_tx_samples.begin(), g_tx_samples.begin() + fft_n_len, g_fft_complex->get_inbuf());
         ...
         ++it, ++idx)
   {
		...
		std::cout << "it:" << *it << ", idx:" << idx << std::endl;
   }
